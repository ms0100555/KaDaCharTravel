    ...
    <nav class="navbar-default navbar-static-side" role="navigation">
      <div class="sidebar-collapse">
        <ul class="nav metismenu" id="side-menu">
          ...
          <li class="active">
              <a href="view.php"><i class="fa fa-database" style="font-size:18px;"></i> <span class="nav-label" style="font-size:18px;">資料維護</span></a>
                  ...
                  <table class="table table-striped table-bordered table-hover dataTables-example" >
                    <tbody>
                      <?php
                        ...
                        $data = sqlQry("SELECT * FROM food WHERE Id = ?",array($ID));
                        echo " <h1>".$data[0]['Name']."</h1>";
                        echo "<td> <a href = 'foodupdate.php?ID=".$ID." & Name=".$data[0]['Name']."& Photo=".$data[0]['Picture1']."& Phone=".$data[0]['Tel']."& Add=".$data[0]['Add']."& Content=".$data[0]['Description']."& Lat=".$data[0]['Px']."& Lng=".$data[0]['Py']."'> 修改景點資訊 </a> </td>";

                        echo "<table align = 'center' border = '1' width = '400' height ='600' >";
                        echo "<tr>";
                        echo "<td colspan = '3' align = 'center'>";
                        echo "<img src= ".$data[0]['Picture1']." height='300' width='350'".">"."</td>";
                        echo "</tr>";

                        echo "<tr>";
                        if($data[0]['Tel'] == ""){
                          echo "<td width = '200' height='50'>電話：暫無資料</td>";
                        }else{
                          echo "<td width = '200' height='50'>電話：".$data[0]['Tel']."</td>";
                        }
                        echo "<td width = '200' height='50'>"."地址：".$data[0]['Add']."</td>";
                        echo "</tr>";

                        echo "<tr>";
                        echo "<td width = '200' height='50'>"."經度：".$data[0]['Px']."</td>";
                        echo "<td width = '200' height='50'>"."緯度：".$data[0]['Py']."</td>";
                        echo "</tr>";

                        echo "<tr>";
                        if($data[0]['Description'] == ""){
                          echo "<td colspan = '2'>簡介：暫無資料</td>";
                        }else{
                          echo "<td colspan = '2'>簡介：<br>".$data[0]['Description']."</td>";
                        }
                        echo "</tr>";
                      ?>
                    </tbody>
